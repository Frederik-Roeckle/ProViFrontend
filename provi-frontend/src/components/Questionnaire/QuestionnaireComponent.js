    ...
    <div className="bg-white p-6 shadow-md rounded-md h-full w-full flex flex-col gap-4">
      ...
      {currentQuestion ? (
        <div className="flex flex-col gap-4 h-full">
          ...
          {(currentQuestion['Answer Type'] === 'multiple choice' || currentQuestion['Answer Type'] === 'follow-up question') && (
                  ...
                  <div key={index} className="flex items-center justify-between gap-2">
                    ...
                    >
                      ...
                      {[...Array(currentQuestion['Options'].split('|').length)]
                        .map((_, i) => i + 1)
                        .map((order) => (
                          <option key={order} value={order} disabled={getUsedNumbers().includes(
                            order.toString()
                          )}>
                            {order}
                          </option>
